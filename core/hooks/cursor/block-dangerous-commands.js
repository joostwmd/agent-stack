 ...
 * Adapted from karanb192/claude-code-hooks. Blocks dangerous patterns before execution.
 *
 ...
 *
 * Logs blocked attempts to ~/.cursor/hooks-logs/
 *
 * Setup: add to Cursor hooks config with matcher "Bash"
 */
...
const LOG_DIR = path.join(os.homedir(), '.cursor', 'hooks-logs');
...
async function main() {
  ...
  try {
    ...
    if (toolName !== 'Bash') return console.log('{}');

    const toolInput = data.tool_input ?? data.toolInput ?? data.input;
    const cmd = typeof toolInput === 'string' ? toolInput : (toolInput?.command ?? toolInput?.command_line ?? '');
